...
class Schrodinger(object):
    ...
    def _compute_k_from_x(self):
        self.psi_mod_k = fft(self.psi_mod_x)

    def _compute_x_from_k(self):
        self.psi_mod_x = ifft(self.psi_mod_k)

    def _time_step(self, dt, Nsteps = 1):
        self.dt = dt
...
def init():
    ...
    psi_k_line.set_data([], [])
    title.set_text("")
    return (psi_x_line, V_x_line,center_line, time_text, psi_k_line,title)

def animate(i):
    global temp
    S._time_step(dt, N_steps)
    psi_x_line.set_data(S.x, 4 * abs(S.psi_x))
    V_x_line.set_data(S.x, S.V_x)
    center_line.set_data(2 * [x0 + S.t * p0 / m], [0,1])
    temp += 1
    time_text.set_text(time_template % (temp * 30 / 1000.0))
    
    psi_k_line.set_data(S.k, abs(S.psi_k))
    title.set_text("t = %.2f" % S.t)
    return (psi_x_line, V_x_line,center_line, time_text, psi_k_line, title)

anim = animation.FuncAnimation(fig, animate, init_func = init, frames = frames, interval = 0.1, blit = True)
